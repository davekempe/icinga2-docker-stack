...
class DirectorBasket:
    ...
    def commandArgument(self,
        ...
        # TODO: The argument needs to be the the same format as the user, currently it is --foo_bar instead of --foo-bar. get the right value
        self.director_basket["Command"][command_name]["arguments"][argument] = _arg

    def template(self, name: str, command_name = None, disabled: bool = False, imports: list = [], template_type: str = DirectorTemplateType.NOTIFICATION.value, timeout: str = None):
        ...
        if timeout:
            self.director_basket[template_type][name]['timeout'] = timeout
        if command_name:
            self.director_basket[template_type][name]['check_command'] = command_name
        if imports:
            self.director_basket[template_type][name]['imports'] = imports

    def templateFields(self, template_name, id, template_type: str = DirectorTemplateType.NOTIFICATION.value, is_required="n", var_filter=None):
        _field = {
            "datafield_id": id,
...
class DirectorBasketNotificationCommand(DirectorBasket):
    ...
    def build(self):
        # TODO: How does this get the PluginDir? 
        ...
        for arg in self.args.keys():
            ...
            command_type = command_dict.get('type', 'str')
            if not command_type:
                command_type = 'str'
            self.datafield(
                id=self.id,
                ...
                datatype=DataFieldType[command_type].value
            )
...
class DirectorBasketCheckCommand(DirectorBasket):
    ...
    def __init__(self, name: str, icinga_var_prefix: str = None, command: str = '', command_name: str = None, id: int = 1111, args: list = []):
        ...
        if self.command_name is None:
            ...
            self.command_name = f"{ObjectPrefix.COMMAND.value}_{self.icingaSafeName(self.name)}"
        self.id = id
        self.args = args
        self.command_path = command
        self.build()
    ...
    def build(self):
        ...
        for arg in self.args.keys():
            ...
            # TODO: when we have a group with a argument, eg: mode, we create a new template. That template should have the value it needs as well.
